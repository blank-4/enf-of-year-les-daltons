<!DOCTYPE html>

<html>
    <head>
        <meta charset="utf-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title>Mots Melax</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" type="text/css" media="screen" href="css/main.css" />
        <link rel="stylesheet" href="css/mobiledesign.css" type="text/css" media="screen and (max-width: 450px)">
        <link href="https://fonts.googleapis.com/css?family=Nova+Flat" rel="stylesheet">
        <link href="https://fonts.googleapis.com/css?family=Grand+Hotel" rel="stylesheet">
        <link href="https://fonts.googleapis.com/css?family=ZCOOL+QingKe+HuangYou" rel="stylesheet"> 
        <script src="http://code.jquery.com/jquery-latest.min.js" type="text/javascript"></script>
    </head>
    <body>
        <div id='cssmenu'>
            <ul>
                <li><a href='../Page-accueil/index.php'><span>Accueil</span></a></li>
                <li><a href='../Page-inscription-connexion/connexion.php'><span>Connexion</span></a></li>
                <li class='active'><a href='../Page-Game/game.php'><span>Jouer</span></a></li>
                <li><a href='../Page-Score/score.php'><span>Scores</span></a></li>
            </ul>
        </div>
        <header>
            <h1>Mots Melax</h1>
        </header>
        <div class="wrap-gif">
            <main>
                <div id="grid">
                    <div class="box" data-word="Chat">
                        C
                    </div>
                    <div class="box" data-word="Chat">
                        H
                    </div>
                    <div class="box" data-word="Chat">
                        A
                    </div>
                    <div class="box" data-word="Chat">
                        T
                    </div>
                    <div class="box">
                        D
                    </div>
                    <div class="box">
                        K
                    </div>
                    <div class="box">
                        M
                    </div>
                    <div class="box" data-word="Briquet">
                        B
                    </div>
                    <div class="box" data-word="Basket">
                        T
                    </div>
                    <div class="box">
                        C
                    </div>
                    <div class="box" data-word="Foot">
                        F
                    </div>
                    <div class="box">
                        D
                    </div>
                    <div class="box">
                        F
                    </div>
                    <div class="box">
                        B
                    </div>
                    <div class="box">
                        V
                    </div>
                    <div class="box" data-word="Briquet">
                        R
                    </div>
                    <div class="box" data-word="Basket">
                        E
                    </div>
                    <div class="box" data-word="Pomme">
                        P
                    </div>
                    <div class="box">
                        J
                    </div>
                    <div class="box" data-word="Foot">
                        O
                    </div>
                    <div class="box" data-word="Lait">
                        T
                    </div>
                    <div class="box">
                        Y
                    </div>
                    <div class="box">
                        J
                    </div>
                    <div class="box" data-word="Briquet">
                        I
                    </div>
                    <div class="box" data-word="Basket">
                        K
                    </div>
                    <div class="box">
                        N
                    </div>
                    <div class="box" data-word="Pomme">
                        O
                    </div>
                    <div class="box" data-word="Lait">
                        I
                    </div>
                    <div class="box" data-word="Foot">
                        O
                    </div>
                    <div class="box">
                        L
                    </div>
                    <div class="box">
                        N
                    </div>
                    <div class="box" data-word="Briquet">
                        Q
                    </div>
                    <div class="box" data-word="Basket">
                        S
                    </div>
                    <div class="box">
                        I
                    </div>
                    <div class="box" data-word="Lait">
                        A
                    </div>
                    <div class="box" data-word="Pomme">
                        M
                    </div>
                    <div class="box">
                        F
                    </div>
                    <div class="box" data-word="Foot">
                        T
                    </div>
                    <div class="box">
                        P
                    </div>
                    <div class="box" data-word="Briquet">
                        U
                    </div>
                    <div class="box" data-word="Basket">
                        A
                    </div>
                    <div class="box" data-word="Lait">
                        L
                    </div>
                    <div class="box">
                        N
                    </div>
                    <div class="box">
                        E
                    </div>
                    <div class="box" data-word="Pomme">
                        M
                    </div>
                    <div class="box">
                        O
                    </div>
                    <div class="box">
                        X
                    </div>
                    <div class="box" data-word="Briquet">
                        E
                    </div>
                    <div class="box" data-word="Basket">
                        B
                    </div>
                    <div class="box">
                        G
                    </div>
                    <div class="box">
                        L
                    </div>
                    <div class="box">
                        D
                    </div>
                    <div class="box">
                        D
                    </div>
                    <div class="box" data-word="Pomme">
                        E
                    </div>
                    <div class="box">
                        V
                    </div>
                    <div class="box" data-word="Briquet">
                        T
                    </div>
                    <div class="box" data-word="Singe">
                        S
                    </div>
                    <div class="box" data-word="Singe">
                        I
                    </div>
                    <div class="box" data-word="Singe">
                        N
                    </div>
                    <div class="box" data-word="Singe">
                        G
                    </div>
                    <div class="box" data-word="Singe">
                        E
                    </div>
                    <div class="box" >
                        A
                    </div>
                    <div class="box" >
                        I
                    </div>
                    <div class="box" >
                        M
                    </div>
                </div>
                <div id="listContainer">
                    <!-- <h2>Mots Melax</h2> -->
                    <p>Voici la liste des mots à trouver :</p>
                    <ul id="list">
                        <li>Chat</li>
                        <li>Pomme</li>
                        <li>Briquet</li>
                        <li>Lait</li>
                        <li>Singe</li>
                        <li>Foot</li>
                        <li>Basket</li>
                    </ul>
                    <div id="restart">
                        <h3 id="h3-restart">Bravo vous avez gagné !</h3>
                        <button id="btn-restart"><a href="">Rejouer !</a></button>
                        <p>OU</p>
                        <button id="btn-new-grid"><a href="">Rejouer avec une nouvelle grille !</a></button>
                    </div>
                </div>  
            </main>
        </div>
        <footer>
            <p>Copyright © 2019 - Les Daltons</p>
        </footer>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/fastclick/1.0.6/fastclick.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
        <script type="text/javascript" src="js/main.js"></script>
    </body>
</html>